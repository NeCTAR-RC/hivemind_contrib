...
from hivemind_contrib import show
...
@task
def show_allocation(instance_uuid, connection=None, debug=False):
...
@task
def clean_allocation(instance_uuid, hypervisor, connection=None,
    ...
    if not dups:
        ...
        for i in (dups):
            try:
                ...
                _del_all_allocations_for_instance(connection, i[0], dry_run)
                continue
            except:
...
def _del_all_allocations_for_instance(connection, id, dry_run):
    if dry_run:
        print('{} not found, would delete all allocations'.format(id))
    else:
        connection = get_connection(connection)
        engine = connect(connection, debug)
        db_conn = engine.connect()
...
def _del_dup_allocations_for_instance(connection, id, host, dry_run):
    if dry_run:
       print('would call clean_allocation({}, {}, {})'.format(
           id, host, dry_run))
    else:
