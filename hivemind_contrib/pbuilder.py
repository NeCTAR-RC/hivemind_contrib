...
def get_build_env(os_release, ubuntu_release=None):
    ...
    return f"{ubuntu_release}-{os_release}"


def apt_key_recv_key(key_id, keyring):
    local(
        f"apt-key --keyring {keyring} adv "
...
def build_trusted():
    ...
    with tempfile.NamedTemporaryFile() as tmp_gpg:
        ...
        tmp_gpg.flush()
        local(
            f"gpg --no-default-keyring --keyring {tmp_gpg.name} --export "
            f"| gpg --no-default-keyring --keyring {db} --import"
        )
    with tempfile.NamedTemporaryFile() as tmp_gpg:
...
mirrors = {
    ...
    'jammy-2023.1': [
        ...
        "deb " + NECTAR_REPO + " jammy main",
        "deb " + NECTAR_REPO + " jammy-2023.1 main",
        "deb " + NECTAR_REPO + " jammy-2023.1-testing main",
        "deb " + NECTAR_REPO + " jammy-testing main",
        "deb " + UBUNTU_MIRROR + " jammy-updates main universe",
    ],
    'jammy-2023.2': [
        ...
        "deb " + NECTAR_REPO + " jammy main",
        "deb " + NECTAR_REPO + " jammy-2023.2 main",
        "deb " + NECTAR_REPO + " jammy-2023.2-testing main",
        "deb " + NECTAR_REPO + " jammy-testing main",
        "deb " + UBUNTU_MIRROR + " jammy-updates main universe",
    ],
    'jammy-2024.1': [
    ...
    'noble-2024.1': [
        ...
        "deb " + NECTAR_REPO + " noble-2024.1-testing main",
        "deb " + NECTAR_REPO + " noble-testing main",
        "deb " + UBUNTU_MIRROR + " noble-updates main universe",
    ],
}
...
def pbuilder_env(os_release, name=None, ubuntu_release=None):
    ...
    if name:
        dist_release = f'{dist_release}-{name}'
    output_dir = os.path.join(package_export_dir(), dist_release)
    return shell_env(
        ARCH=ARCH, DIST=dist_release, GIT_PBUILDER_OUTPUT_DIR=output_dir
    )
...
def create(os_release=None, extra_mirror=None, name=None, ubuntu_release=None):
    ...
    dist_release = get_build_env(os_release, ubuntu_release)
    path = f'/var/cache/pbuilder/base-{dist_release}'
    if name:
        path = f'{path}-{name}'
    path = f'{path}-{ARCH}.cow'

    if os.path.exists(path):
        raise Exception(f'PBuilder base image already exists at {path}')
    ...
    with shell_env(ARCH=ARCH, DIST=ubuntu_version):
        local(
            'git-pbuilder create --basepath {basepath}'
            ' --mirror {mirror}'
