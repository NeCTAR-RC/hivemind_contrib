...
def get_freshdesk_config(api_key=None,
                         domain='support.nectar.org.au'):
    """fetch freshdesk API details from config file"""
    msg = " ".join(["No FreshDesk API key found in your hivemind config file.",
                    "Please set api_key value in",
...
def get_neutron_client():
    ...
    return neutron_client.Client('2.0', **args)


def _find_flavor(nc, flavor_id):
    """Get a flavor ID"""
    return nc.flavors.get(flavor_id)


def _find_image(nc, image_id):
    """Get an image by ID"""
    return nc.images.get(image_id)


def _find_user(kc, user_id):
    return kc.users.get(user_id)


def _find_tenant(kc, tenant_id):
    return kc.tenants.get(tenant_id)
...
def _generate_instance_info(instance_id, style=None):
    ...
    try:
        info['flavor'] = '%s (%s)' % (_find_flavor(nc, flavor_id).name,
                                      flavor_id)
    except Exception:
    ...
    if tenant_id:
        try:
            tenant = _find_tenant(kc, tenant_id)
...
def get_ticket_recipients(instance):
    ...
    kc = keystone.client()
    user = _find_user(kc, instance.user_id)
    if user.email:
        email_addresses.append(user.email)

    # Add tenant members to ticket recipient list
    tenant = _find_tenant(kc, instance.tenant_id)
