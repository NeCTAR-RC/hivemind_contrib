...
def setup_project(name, org_name='NeCTAR-RC', fork_from=None,
    ...
    if fork_repo:
        try:
            ...
            pass
    if org_name == 'internal':
        local('git remote add origin git@git.melbourne.nectar.org.au:internal/%s' % name)
        local('git push ssh://%s@review.rc.nectar.org.au:29418/%s master' % (
            gerrit_user, full_name))
    else:
        try:
