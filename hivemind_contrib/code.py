...
@task
@verbose
def setup_project(name, org_name='NeCTAR-RC', fork_from=None, openstack_version=None, team_id=338031):
    ...
    try:
        ...
        pass
    local('git fetch --all')
    local('git checkout -b nectar/%s openstack/stable/%s' % (openstack_version, openstack_version))
