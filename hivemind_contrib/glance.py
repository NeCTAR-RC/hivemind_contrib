...
def image_ready_wp4(image):
    ...
    for attr in wp4PropertyList:
        ...
        elif image.properties.has_key(attr):
    ...
    if imageReady:
        ...
        print("Image {} does not comply with community standards, please fix..".format(image.name))
        #return False
	return True
...
def community_archive(image_id, dry_run=True):
    """Archive an EOL Community image by moving it to the Community Archive tenant."""
    if dry_run:
        print("Running in dry run mode")
    ...
    if dry_run:
        print("Would archive image {} ({}) to tenant {} ({})"
              .format(image.name, image.id,
                      community_archive_tenant.name, community_archive_tenant.id))
        if 'murano_image_info' in image.properties:
            print('Would remove murano image properties from {}'
                  .format(image.id))
    else:
