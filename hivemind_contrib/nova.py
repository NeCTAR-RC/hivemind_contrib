...
def all_servers(client, zone=None, ip=None, project=None, user=None,
                limit=None, host=None, status=None, image=None):
    marker = None
    opts = {}
    ...
    if user:
        opts['user_id'] = extract_account(user).id
...
def extract_server_info(server):
    ...
    try:
        ...
        server_info['project_name'] =\
                extract_project(server_info['project']).name

        server_info['accessIPv4'] = extract_ip(server)
        user = extract_account(server_info['user'])
...
def extract_account(user):
    ks = keystone.client()
    user = keystone.get_user(ks, user)
    return user


def extract_project(project):
    ks = keystone.client()
    project = keystone.get_project(ks, project)
    return project
...
def parse_dash(range_str):
    ...
    return range(int(hosts[0]), int(hosts[1]) + 1) if len(hosts) > 1 \
                 else [range_str]
...
@task
@decorators.verbose
def list_instances(zone=None, nodes=None, project=None, user=None, status=None,
