...
def _scenario_compute_failure(novaclient, server, changes_since):
    try:
        ...
        action_info = get_instance_action(novaclient, server['id'],
                                          last_action.request_id)
        if _normalize_time(action_info.start_time) >= _normalize_time(
            changes_since) and action_info.action == "stop" and \
                               not action_info.project_id and \
                               not action_info.user_id:
            return True
        else:
            return False
...
def list_instances(zone=None, nodes=None, project=None, user=None,
    ...
    if scenario:
        ...
        result = [server for server in result if func(novaclient, server, changes_since)]
        if not result:
