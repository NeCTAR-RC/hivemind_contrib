...
def update_allocation_home(project, institute_domain):
    ...
    if (
        'allocation_home' in proj_dict
        and proj_dict['allocation_home'] is not None
    ):
    ...
    if institute_domain not in homes:
        ...
        set_project_metadata(project, 'allocation_home', ",".join(homes))
...
def get_members(keystone, project, roles=None):
    ...
    if not roles:
        role_assignments = keystone.role_assignments.list(
            project=project.id, include_names=True
        )
    else:
        role_assignments = []
        for role in roles:
            role_assignments.extend(
                keystone.role_assignments.list(
    ...
    for ra in role_assignments:
        if ra.user['id'] in users:
            ...
            users[ra.user['id']]['project_name'] = ra.scope['project']['name']
    return users
...
def print_members(keystone, project, roles=None):
    ...
    print(f"Members of {project.name} ({project.id}):")
    print(str(table))
    return users
    ...
    if select_roles:
        ...
        for role in select_roles:
            try:
                ...
                error(f"Specified role [{role}] is not found")
    else:
        role_ids = None
...
def remove_project_roles(project, user, roles, quiet=False):
    ...
    for role in roles:
        role = keystone.roles.find(name=role)
        try:
            keystone.roles.revoke(
                user=user.id, project=project.id, role=role.id
            )
        except NotFound:
            pass
    if not quiet:
...
def user_projects(user, show_disabled=False):
    ...
    print(f"Projects and roles for user {user.name}:")
    print(str(table))
...
def add_bot_account(project, user, suffix='bot'):
    ...
    bot_name = f"{project.name}_{suffix}"
    ...
    table = PrettyTable(
        ["User ID", "Username", "Password", "Proxy owner", "Proxy owner ID"]
    )
    table.add_row(
        [new_user.id, new_user.name, password, real_user.name, real_user.id]
    )
    print(str(table))
...
def users_mailing_list(output):
    ...
    print(f'CSV file written to {output}')
