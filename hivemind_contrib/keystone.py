...
def add_robot_account(tenant, user):
    ...
    table = PrettyTable(["User ID", "Username", "Password", "Proxy owner", "Proxy owner ID"])
    table.add_row([new_user.id, new_user.name, password, tenant_manager.name, tenant_manager.id])
    print(str(table))
