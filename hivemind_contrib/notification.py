    ...
    for instance in instances:
        if instance['project'] in project.keys():
            ...
            if cclist:
                ...
                project[instance['project_name']].update({'recipients': cclist})
...
def freshdesk_mailout(template, zone=None, ip=None, nodes=None, image=None,
    ...
    """Mailout announcements from freshdesk (Recommended).
       Freshdesk tickets per project will be created along with outbound email.
       ...
       the ticket and status is changed to Open.
    ...
    if dry_run:
        ...
        if not query_yes_no(query % (len(email_files), len(email_files)/60 + 1),
                            default='no'):
            sys.exit(1)
