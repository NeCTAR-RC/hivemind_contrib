...
def _list_short(records, limit=None, lead_ip=None):
    ...
    table = [
        record['ip'] for record in records[lead_pos:] if 'type' not in record
    ]
...
def _list_group(table, records, lead_ip=None):
    ...
    for record in records[lead_pos + 1 :]:
        ...
        pip = prev['ip']
        if not matching:
            match = SequenceMatcher(None, rr, pr)
...
def _get_ptr_name_zone(ip_address):
    ...
    name = f"{ip_address.reverse_pointer}."
...
def _create_CNAME_record(
    client, zone, name, record, description=None, ttl=None, yes_prompt=False
):
    """Create CNAME record IPv4."""
    record_name = f"{name}.{zone}"
    ...
    if entry is not None:
        _show(entry)
        if yes_prompt or (
            click.confirm("Entry existed. Update entry instead?", abort=True)
        ):
            click.echo(f"Updating CNAME record for {record_name}")
            update_data = _make_record_update_data(record, description, ttl)
            _show(client.recordsets.update(zone, record_name, update_data))
    else:
        click.echo(f"Creating CNAME record for {record_name}")
        _show(
            client.recordsets.create(
                zone, record_name, 'CNAME', [record], description, ttl
            )
        )


def _create_A_and_PTR_record(
    client, zone, name, record, description=None, ttl=None, yes_prompt=False
):
    ...
    if entry is not None:
        ...
        if yes_prompt or (
            ...
            click.echo(f"Updating A record for {record_name}")
            old_record_ip = ipaddress.ip_address(entry['records'][0])
            old_ptr_record_name, old_ptr_zone = _get_ptr_name_zone(
                old_record_ip
            )
            update_data = _make_record_update_data(
                record.exploded, description, ttl
            )
            _show(client.recordsets.update(zone, record_name, update_data))
            ptr_entry = try_assign(
                client.recordsets.get, old_ptr_zone, old_ptr_record_name
            )
            if ptr_entry is not None:
                _show(ptr_entry)
                if yes_prompt or (
                    click.confirm("Delete old PTR record?", abort=True)
                ):
                    click.echo(
                        f"Deleting PTR record for {old_ptr_record_name}"
                    )
                    _show(
                        client.recordsets.delete(
                            old_ptr_zone, old_ptr_record_name
                        )
                    )

    else:
        click.echo(f"Creating A record for {record_name}")
        _show(
            client.recordsets.create(
                zone, record_name, 'A', [record.exploded], description, ttl
            )
        )
    ...
    if ptr_entry is not None:
        ...
        if yes_prompt or (
            ...
            click.echo(f"Updating PTR record for {ptr_record_name}")
            update_data = _make_record_update_data(
                record_name, description, ttl
            )
            _show(
                client.recordsets.update(
                    ptr_zone, ptr_record_name, update_data
                )
            )
    else:
        click.echo(f"Creating PTR record for {ptr_record_name}")
        _show(
            client.recordsets.create(
                ptr_zone,
                ptr_record_name,
...
def _generate_free_entry(addr, records_dict):
    ...
    if addr in records_dict:
        ...
        return {'ip': addr, 'records': ['No DNS record found']}
...
def _find_free_entries(client, network, zone):
    ...
    records_dict = {
        '.'.join(record['name'][:-14].split('.')[::-1]): record
    ...
    records.insert(
        0, _generate_free_entry(network.network_address, records_dict)
    )
    records.append(
        _generate_free_entry(network.broadcast_address, records_dict)
    )
    return records
...
@click.option(
    '--record', required=True, type=str, help='The record of the DNS entry'
)
...
def create(zone, name, type, record, description, ttl, yes):
    ...
    if type in 'A':
        ...
        _create_A_and_PTR_record(
            ds, zone, name, ip_address, description, ttl, yes
        )
    elif type in 'CNAME':
...
@click.argument('name')
@click.option(
    '--yes', is_flag=True, help='Say YES to all delete prompt (DANGEROUS)'
)
def delete(zone, name, yes):
    ...
    if record['type'] in 'A':
        ...
        if yes or click.confirm(
            "Are you sure you want to delete this record?", abort=True
        ):
            click.echo(f"Deleting A record for {record_name}")
            ...
            ptr_record = try_assign(
                ds.recordsets.get, old_ptr_zone, old_ptr_record_name
            )
            if ptr_record is not None:
                _show(ptr_record)
                if yes or click.confirm(
                    "Do you want to delete PTR record?", abort=True
                ):
                    print(f"Deleting PTR record for {old_ptr_record_name}")
                    _show(
                        ds.recordsets.delete(old_ptr_zone, old_ptr_record_name)
                    )
    # Delete CNAME record
        ...
        if yes or click.confirm(
            ...
            click.echo(f"Deleting CNAME record for {record_name}")
            _show(ds.recordsets.delete(zone, record_name))
...
def free(network, display, limit):
    ...
    raw_network = network
    network = try_assign(
        ipaddress.ip_network, network, strict=False, exit=True
    )
