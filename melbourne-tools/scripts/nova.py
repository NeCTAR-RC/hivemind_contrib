...
def _find_hosts_in_azs(nova, zones, hosts):
    ...
    if not hosts:
        return set(
            ...
            and aggregate.metadata["availability_zone"] in zones
        )
    else:
        return set(
            host
...
def _find_projects(allocation, allocation_home):
    ...
    opts = {'parent_request__isnull': True, 'allocation_home': allocation_home}
    allocations = allocation.allocations.list(**opts)

    return set(a.project_id for a in allocations)
...
def _find_instances(nova, hosts, statuses, project_ids):
    ...
    for query in queries:
        yield from nova.servers.list(search_opts=query)
...
def _list_instances(
    ...
    if exclude_aggregates:
        exclude_hosts = _find_hosts_in_aggregates(
            nova, exclude_aggregates, exclude_hosts
        )
    if exclude_availability_zones:
        exclude_hosts = _find_hosts_in_azs(
            nova, exclude_availability_zones, exclude_hosts
        )
    ...
    for ins in instances:
        ...
        for f in fields:
            if 'flavor' in f:
                ...
                row.append(
                    ', '.join(
                        sc['name'] for sc in ins._info['security_groups']
                    )
                )
            elif f in 'image':
...
def _render_table_instance(instance):
    ...
    fields.extend(f"{az} network" for az in instance._info['addresses'].keys())
...
def _recommend_nmap_command(ports, ip):
    ...
    click.echo(f"Globally opened ports found: {ports}")
    command = "nmap -sV -sT -sC -p {} {}".format(','.join(ports), ip)
    return command
...
@cli.command()
@click.option(
    '--host', help='Only list instances from HOST (eg. qh2-rcc[10-99])'
)
@click.option(
    '--exclude-host', help='Exclude instances from HOST(eg. qh2-rcc[10-99])'
)
@click.option(
    '-s', '--status', multiple=True, help='Only list instances with STATUS'
)
@click.option(
    '-az',
    '--availability-zone',
...
@click.option(
    '--exclude-aggregate', multiple=True, help='Exclude instances in AGGREGATE'
)
@click.option(
    '-lc',
    '--last-changed',
...
@click.option(
    '--project', multiple=True, help='Only list instances from PROJECT_ID'
)
@click.option(
    '-c',
    '--column',
...
@click.option('--allocation-home', help='Filter by an ALLOCATION_HOME')
def list(
    host=None,
    last_changed=None,
...
@click.option('--allocation-home', help='Filter by an ALLOCATION_HOME')
@click.option(
    '--detail', is_flag=True, help='DETAIL the instance statistic by projects'
)
def stat(
    host=None,
    last_changed=None,
...
def security_investigate(ip):
    ...
    instance._info['os-extended-volumes:volumes_attached'] = ', '.join(
        v['id'] for v in instance._info['os-extended-volumes:volumes_attached']
    )
    for az in instance._info['addresses'].keys():
        network_name = f"{az} network"
        ...
        for net in network:
            ...
            if net['addr'] in ip:
                ...
                break
        output = ', '.join("{}".format(net['addr']) for net in network)
    ...
    if '22' in nmap_ports:
        ...
        click.echo(f'SSH authentication method list: {ssh_authlist}')
    ...
    channel.execute(
        f"virsh dumpxml {target_instance_name} | grep {target_mac_device} -A3 | grep bridge"
    )
    size, data = channel.read()
    target_bridge_interface = data.split("'")[1]

    tcpdump_command = f"ssh {target_host} 'tcpdump -l -q -i {target_bridge_interface} not arp and not icmp'"
    ...
    click.echo(f"* Discover VM running services by running: {nmap_command}")
    click.echo(f"* Discover VM network traffics by running: {tcpdump_command}")
...
def aggregate_move_host(aggregate, host):
    ...
    for host in hosts:
        if not host_ag_mapping[host]:
            click.echo(
                f"ERROR: Unable to move host. {host} not in any aggregate."
            )
            continue
        target = "{}_{}".format(
            host_ag_mapping[host][0].name.split('_')[0], aggregate
        )
        if target not in target_ag:
            click.echo(f"ERROR: {target} not found.")
            continue
        for ag in host_ag_mapping[host]:
            nova.aggregates.remove_host(ag, host)
        click.echo(
            "{} removed from [{}]".format(
                host, ', '.join(ag.name for ag in host_ag_mapping[host])
            )
        )
        click.echo(f"{host} added to {target}")
